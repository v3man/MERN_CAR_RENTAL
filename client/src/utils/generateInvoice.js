 ...
 * @param {Object} booking - The booking object from the backend.
 * @param {Object} car - The car object details.
 */
export const generateInvoice = (booking, car) => {
  const doc = new jsPDF();
  const invoiceNumber = `INV-${booking._id.slice(-6).toUpperCase()}`;
  ...
  doc.autoTable({
    ...
    body: [
      [`Car Rental (${car?.brand} ${car?.model})`, `${days} Days`, `$${car?.pricePerDay}`, `$${booking.totalPrice}`],
    ],
    foot: [
